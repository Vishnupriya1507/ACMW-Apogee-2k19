...
def process(data,user_,request):
    ...
    if data['board_state']==True:
        ...
        response = {}
        count = (7200-request.user.time)/(request.user.currentQs*30)
        print("request.user.time : ", request.user.time," ","request.user.currentQs*40 : ", request.user.currentQs*40)
        if(count >=1 ):
            print("count is >= 1")
