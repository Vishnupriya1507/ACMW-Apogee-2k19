...
def cda(request):
    ...
    return render(request, 'cda.html', {'games': games,'score':request.user.score})
...
def placements(request):
    ...
    return render(request, 'placements.html', {'games': games})
...
def endscore(request):
    print(request,request.user)
    if request.method == 'POST':
        data = json.loads(request.body)
        print(data)
        response = {}
        response['fscore'] = request.user.score
...
def checkAnswer(request):
    ...
    if request.method == 'POST':
        ...
        if qs.solution==answer:
            ...
            request.user.score+=(50 * request.user.currentQs)
